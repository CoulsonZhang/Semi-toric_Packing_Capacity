...
def show_graph(Vertices, sl2_length, set, tittle=""):
    ...
    for i in range(len(set)):
        if (set[i] != 0):
            ...
            forward_vertex = np.array(Vertices[i]) + (set[i] / sl2_length[i]) * np.array(
                [Vertices[forward][0] - Vertices[i][0], Vertices[forward][1] - Vertices[i][1]])
            backward_vertex = np.array(Vertices[i]) + (set[i] / sl2_length[backward]) * np.array(
                [Vertices[backward][0] - Vertices[i][0], Vertices[backward][1] - Vertices[i][1]])
...
def get_info(Vertices, all=False):
    ...
    print("This is the array of SL2 length: \n{} \n\nFollowing shows the Packing vertices & its magnitude: ".format(
        sl2_length))
